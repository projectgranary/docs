...
![Example data flow for a notifaction](../../../.gitbook/assets/reaper_dev.png)
...
The Reaper runs peridodically configured via a CRON job. Depending on the next Reaper run but earliest after the five minutes, the Reaper will emit our notification grain to the the topic `grnry_data_in_ttl-feedback` .
...
```java
topic: "grnry_data_in_ttl-feedback"
...
To act upon a notification, we need to deploy a belt that consumes from the topic `grnry_data_in_ttl-feedback`. In the belt callback script, the values in the `execute` function will look like this:
