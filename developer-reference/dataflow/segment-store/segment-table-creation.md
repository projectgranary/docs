...
The creation job runs in a container and can be deployed via [Segment Management API](../../api-reference/segment-management-api.md).

The following table comprises a complete list of variables \(required if there is no default\):
...
| `DB_HOST` | database endpoint | `grnry-pg-citus-master-0` |
...
| `CITUS_DIST_COL` | name of the column the source table is and the target segment will be distributed by. currently, this has to be a single column \(only mandatory if `DB_TYPE = citus`\) | correlation\_id |
